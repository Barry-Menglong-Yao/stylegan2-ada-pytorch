...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, sync,inject_info):
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img = self.G_synthesis(ws,inject_info)
        return img, ws
    ...
    def run_Encoder(self, img, c, sync):
        ...
        if self.augment_pipe is not None:
            img = self.augment_pipe(img)
        with misc.ddp_sync(self.D , sync):
            logits,generated_z ,mu,log_var,inject_info = self.D(img, c,"encoder")
        return logits,generated_z ,mu,log_var,inject_info
...
class GANVAELoss(StyleGAN2Loss):
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain,model_attribute):
    ...
    def min_reconstruct_loss(self,real_img,real_c,sync,gain):
        with torch.autograd.profiler.record_function('Emain_forward'): 
            logits,gen_z_of_real_img ,mu,log_var,inject_info = self.run_Encoder(real_img, real_c, sync=False)
            reconstructed_img, _ = self.run_G(gen_z_of_real_img, real_c, sync=(sync),inject_info=inject_info) 
    ...
    def maximize_gen_logits(self,gen_z,gen_c,sync,do_Gpl ,gain):
        with torch.autograd.profiler.record_function('Gmain_forward'):
            gen_img, _gen_ws = self.run_G(gen_z, gen_c, sync= sync,inject_info=None) # May get synced by reconstruct.
    ...
    def apply_gpl_regularization(self,gen_z, gen_c,sync,gain ):
        with torch.autograd.profiler.record_function('Gpl_forward'):
            ...
            gen_img, gen_ws = self.run_G(gen_z[:batch_size], gen_c[:batch_size], sync=sync,inject_info=None)
    ...
    def minimize_gen_logits(self,gen_z,gen_c,sync,gain):
        with torch.autograd.profiler.record_function('Dgen_forward'):
            gen_img, _gen_ws = self.run_G(gen_z, gen_c, sync=False,inject_info=None)
    ...
    def maximize_real_logits_min_vae_loss(self,real_img,do_Dr1,real_c,sync,gain,do_Dmain,has_vae_loss):
        ...
        with torch.autograd.profiler.record_function(name + '_forward'):
            ...
            if has_vae_loss:
                real_logits,gen_z_of_real_img ,mu,log_var,inject_info = self.run_Encoder(real_img , real_c,  sync=(sync))
                reconstructed_img, _ = self.run_G(gen_z_of_real_img, real_c, sync=False,inject_info=inject_info) 
                loss = torch.nn.MSELoss(reduction='none')
                loss_Emain_reconstruct = loss(reconstructed_img, real_img)
                loss_Emain_reconstruct=loss_Emain_reconstruct.view(loss_Emain_reconstruct.shape[0],-1)
                loss_Emain_reconstruct=torch.mean(loss_Emain_reconstruct,dim=1)
    ...
    def min_vae_loss(self,real_img, real_c,sync,gain ):
        ...
        with torch.autograd.profiler.record_function(name + '_forward'):
            ...
            VAE_loss,loss_Emain_reconstruct=self.vae_gan.vae_loss(reconstructed_img,real_img,mu,log_var,self.vae_beta,self.vae_alpha_d)
        with torch.autograd.profiler.record_function(name + '_backward'):
            VAE_loss.mul(gain).backward()  
